<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="发送日期" prop="sentDate">
        <el-date-picker clearable
          v-model="queryParams.sentDate"
          type="date"
    ...
    <el-table v-loading="loading" :data="notificationsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="发送日期" align="center" prop="sentDate" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.sentDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除商铺通知编号为"' + ids + '"的数据项？').then(function() {
